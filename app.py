...
4. Preview randomization distributions
...
def process(
    ...
    try:
        ...
        if len(variants) > 3:
            log_lines.append(f"   ... and {len(variants) - 3} more")
    ...
    try:
        ...
        # Load and display annotations summary
        ...
        ann_path = ""
        log_lines.append(f"⚠️ Export failed: {e}")

    # ── Step 5: Visualization ──
    log_lines.append("")
    log_lines.append("=" * 55)
    log_lines.append("STEP 5: Rendering comparison visualization...")
    log_lines.append("=" * 55)

    try:
        comparison_path = render_comparison(variants, f"{config.output_dir}/comparison.png")
        log_lines.append(f"✅ Comparison plot: {comparison_path}")
    except Exception as e:
    ...
    if pose_enabled:
        summary_parts.append(f"  🔩 Object: ±{pose_jitter*1000:.1f}mm / ±{pose_rot:.1f}°")
    ...
    return config_json, summary, comparison_path, "\n".join(log_lines)
...
with gr.Blocks(
    ...
    gr.Markdown(
        ...
        **Domain Randomization for Industrial AI Training Data**
        ...
        Load a USD scene → configure randomization → generate variants → export COCO dataset.
        
        *Part of the [Industrial AI Toolchain](https://github.com) — downstream from spec-to-sim-copilot*
        """
    )
    ...
    with gr.Row():
        with gr.Column(scale=1):
            config_output = gr.Code(
                label="⚙️ SDG Configuration (JSON)",
                language="json",
                lines=18,
            )
    ...
    submit_btn.click(
        ...
        outputs=[config_output, summary_output, preview_output, log_output],
    )
